...
WINDOW *main_win, *win_body, *win_form, *win_menu;
FORM *form;
FIELD **fields;
MENU *menu;
ITEM **items;
		...
		int nb_buttons, char **requests, int nb_fields)
{
	...
	items = (ITEM**) malloc(sizeof(ITEM *) * (nb_buttons+1));
	assert(items);
...
void driver_buttons(ITEM *item)
{
	...
	if (strcmp(name, "OK") == 0) {
		...
		mvprintw(LINES-1, 1, "[*] QUIT clicked, 'F3' to quit\n");
...
int main()
{
    ...
    // Make a box around the whole window
